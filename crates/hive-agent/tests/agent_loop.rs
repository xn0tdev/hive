...
//! turn 1 the model calls a tool, turn 2 it produces a final answer. We assert
//! the tool actually ran and the right events were emitted.
...
use hive_agent::{AgentBuilder, UserInput};
...
use hive_core::AgentEvent;

/// Returns a tool call the first time, then a final message.
struct MockProvider {
    calls: AtomicUsize,
}
