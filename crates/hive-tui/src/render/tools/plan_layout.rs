...
fn wrap_segments(text: &str, width: usize) -> Vec<(usize, String)> {
    ...
    while i < chars.len() {
        ...
        out.push((line_start, text[line_start..end_byte].to_string()));
...
pub fn paint_highlights(
    ...
    for (row, span) in spans.iter().enumerate() {
        ...
        for (a, b, tone) in merged {
            if cursor < a {
                spans_out.push(Span::styled(body[cursor..a].to_string(), base));
            }
            let hi = match tone {
                MarkTone::Pending => pending,
                MarkTone::Noted => noted,
            };
            spans_out.push(Span::styled(body[a..b].to_string(), hi));
            cursor = b;
        }
        if cursor < span.end {
            spans_out.push(Span::styled(body[cursor..span.end].to_string(), base));
        }
