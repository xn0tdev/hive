...
impl CodeBlock {
    ...
    /// Render highlighted source into `area`.
    ///
    /// - `Some(bg)` — fill the panel and keep that background under every
    ///   token (avoids jagged strips from fg-only highlight styles).
    /// - `None` — draw on the terminal background with no panel fill.
